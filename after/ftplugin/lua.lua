...
ft.addMap("i", "<C-r>:", [[histget(':')->substitute('\v^\s*%(lua\=?|h%[elp])\s*', '', '')]], { expr = true })
		...
		local function sourcePluginSpec() -->
			...
			if load then
				...
				if ok then
					...
					if #loaded == 0 then
						vim.notify(
							string.format("No packages reloaded from plugins/%s", vim.fs.basename(bufname)),
							vim.log.levels.INFO
						)
					else
						vim.notify(
					...
					vim.notify(
						string.format("Could not load plugins/%s: %s", vim.fs.basename(bufname), pkgs),
						vim.log.leves.ERROR
					)
				end
			end
