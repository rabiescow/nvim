...
function M.run()
    ...
    for client, lenses in pairs(lenses_by_client) do
        for _, lens in pairs(lenses) do
            if lens.range.start.line == (line - 1) and lens.command and
                lens.command.command ~= '' then
...
function M.clear(client_id, bufnr)
    ...
    local buffers = bufnr and {bufnr} or
                        vim.tbl_filter(api.nvim_buf_is_loaded,
                                       api.nvim_list_bufs())
...
function M.refresh(opts)
    ...
    for _, buf in ipairs(buffers) do
        if not active_refreshes[buf] then
            local params = {textDocument = util.make_text_document_params(buf)}
            active_refreshes[buf] = true

            local request_ids = vim.lsp.buf_request(buf,
                                                    ms.textDocument_codeLens,
                                                    params, M.on_codelens,
                                                    function() end)
